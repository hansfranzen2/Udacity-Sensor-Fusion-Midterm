...
while True:
    try:
        ...
        if 'bev_from_pcl' in exec_list:
            ...
            # I added cnt_frame as an argument for making it available as an image caption
            lidar_bev = pcl.bev_from_pcl(lidar_pcl, configs_det, frame_no=cnt_frame, vis=[False, False, False])
        else:
        ...
        if 'show_pcl' in exec_list:
            # Note: I added cnt_frame as a function argument; makes book keeping easier
            pcl.show_pcl(lidar_pcl, frame_no=cnt_frame)
...
if 'show_detection_performance' in exec_list:
    ...
    # It seems like the second argument is unnecessary (and not present in the function)
