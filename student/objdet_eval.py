...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            w, l, h = label.box.width, label.box.length, label.box.height
            yaw = label.box.heading
            [fl_label, rl_label, rr_label, fr_label] = tools.compute_box_corners(x, y, w, l, yaw)
            box_label = Polygon((fl_label, rl_label, rr_label, fr_label))
            center_label = np.array([x, y, z])
            ...
            for det in detections:
                ...
                [fl_det, rl_det, rr_det, fr_det] = tools.compute_box_corners(x, y, w, l, yaw)
                box_det = Polygon((fl_det, rl_det, rr_det, fr_det))
                center_det = np.array([x, y, z])
                
                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                [dist_x, dist_y, dist_z] = np.abs(center_label - center_det)
                
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                union = box_det.union(box_label)
                intersection = box_det.intersection(box_label)
    ...
    # print("AP = "+str(all_positives))

    ## step 2 : compute the number of false negatives
    false_negatives = len([valid for valid in labels_valid if valid]) - true_positives # FN = (number of all valid ground truths in frame) - TP 
...
def compute_performance_stats(det_performance_all):
    ...
    all_positives = sum([pos_neg[0] for pos_neg in pos_negs])
    true_positives = sum([pos_neg[1] for pos_neg in pos_negs])
    false_negatives = sum([pos_neg[2] for pos_neg in pos_negs])
    false_positives = sum([pos_neg[0] for pos_neg in pos_negs])
